...
@export
def Danmaku2ASS(input_files, input_format, output_file, stage_width, stage_height, reserve_blank=0, font_face=_('(FONT) sans-serif')[7:], font_size=25.0, text_opacity=1.0, duration_marquee=10.0, duration_still=10.0, comment_filter=None, is_reduce_comments=False, progress_callback=None):
    try:
        if comment_filter:
...
if __name__ == '__main__':
    ...
    将它们放在同一目录中，大多数媒体播放器会自动加载它们。对于MPlayer的，必须您指定-ass选项对话。
